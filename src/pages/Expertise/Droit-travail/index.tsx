...
export default function HomeDroitTravail({ droitTravail }) {
    ...
    const [titreCompetence, setTitreCompentce] = useState('');
    ...
    return (
        <div className={"relative"}>
                    ...
                    <div className={"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full lg:w-auto"}>
                        <div className={"flex justify-center"}>
                            <div className="block p-10 bg-white h-auto w-4/5 rounded-xl border border-gray-100 p-4 focus:border-0 shadow transition-all duration-500 hover:ring-1 hover:ring-gray-300 focus:outline-none focus:ring cursor-default ">
                                ...
                                <h2 className="mt-2 font-bold text-lg text-gray-800">{titreCompetence}</h2>
            ...
            <div className={"w-full justify-center flex py-20 min-h-screen bg-white"}>
                <div className={"w-5/6"}>
                    <h2 className={"font-bold text-gray-800 text-4xl uppercase mb-6"}>Mes compétences</h2>
                    <hr className={"mb-5"}/>
                    ...
                    {/**<div className={"grid grid-cols-1 lg:grid-cols-4 gap-8 mt-10"}>
                        {droitTravail.map((currentDroit, index) => {
                            return(
                                <div onClick={(e) => {setDetailsCompetences(currentDroit.Texte); setTitreCompentce(currentDroit.Titre)}} className="block bg-white rounded-xl border border-gray-100 p-4 shadow hover:border-red-700 hover:shadow-red-50 hover:shadow-lg hover:scale-105 transition-all duration-500 hover:ring-1 hover:ring-gray-300 focus:outline-none focus:ring cursor-pointer skill">
                                  <span className="inline-block rounded-lg bg-red-100 p-3">
                                    <svg className="h-6 w-6 text-gray-800" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                      <path d="M12 14l9-5-9-5-9 5 9 5z"></path>
                                      <path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path>
                                    ...
                                    <h2 className="mt-2 font-thin text-gray-800">{currentDroit.Titre}</h2>
                                </div>
                            );
                        })}
                    </div>*/}
