...
import { COMPETENCES_DROIT_TRAVAIL_INDEX_ID, BLOG_INDEX_CACHE } from './server-constants'

export default async function getCompetencesDroitTravailIndex(previews = true) {
    ...
    if (!postsTable) {
        try {
            const data = await rpc('loadPageChunk', {
                pageId: COMPETENCES_DROIT_TRAVAIL_INDEX_ID,
